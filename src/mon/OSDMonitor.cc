                                         ...
                                         stringstream& ss)
{
  ...
  if (var == "size") {
    ...
    p.hit_set_params.reset_params(params);
    ...
    if (p.hit_set_params.get_params()->get_type() != HitSet::TYPE_BLOOM) {
      ss << "hit set is not of type Bloom; invalid to set a false positive rate!";
      return -EINVAL;
    }
    BloomHitSet::Params *bloomp = new BloomHitSet::Params(
	*p.hit_set_params.get_params()->get_as_type<BloomHitSet>());
    bloomp->false_positive = f;
    p.hit_set_params.reset_params(bloomp);
